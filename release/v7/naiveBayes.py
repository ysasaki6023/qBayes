...
def process(column_to_use,fileName):
    ...
    nb.TestByCompany(fpath="analysis/%s.csv"%fileName,topn=25,wordFilter=wd2,verbose=True)

def getColumns():
    columns_to_use = {}
    columns_to_use["idx00_rental"]      = "貸与物の有無"
    columns_to_use["idx01_material"]    = "有形 / 無形（情報/体験/情報＋権利）"
    columns_to_use["idx02_customize"]   = "汎用 / カスタマイズ"
    columns_to_use["idx03_luxury"]      = "必須 / 特徴なし / 贅沢"
    columns_to_use["idx04_brand"]       = "ブランド価値"
    columns_to_use["idx05_experience"]  = "経験曲線"
    columns_to_use["idx06_switchcost"]  = "スイッチングコスト"
    columns_to_use["idx07_usernetwork"] = "顧客蓄積"
    columns_to_use["idx08_infonetwork"] = "情報蓄積"
    columns_to_use["idx09_addictive"]   = "中毒性"
    columns_to_use["idx10_tradecount"]  = "取引相手"
    columns_to_use["idx11_regional"]    = "地域制約"
    columns_to_use["idx12_userdepend"]  = "特定顧客依存"
    columns_to_use["idx13_monetize"]    = "課金形態"
    columns_to_use["idx14_continuous"]  = "継続性"
    columns_to_use["idx15_paytiming"]   = "課金タイミング"
    return columns_to_use

if __name__ == "__main__":
    columns_to_use = getColumns()

    for idx,key in enumerate(columns_to_use):
        column_to_use = columns_to_use[key]
        print key,column_to_use
        process(column_to_use,fileName=key)
