...
class NaiveBayes:
    ...
    def loadCategoryFile(self,fpath,index_column,column_to_use,codec="shift-jis"):
    ...
    def TestByCompany(self,fpath,topn=10,wordFilter=None,verbose=False,codec="shift-jis"):
        ...
        for index in trainData+testData:
            ...
            line = [index,truth,infer[0][0],"%.3f%%"%(infer[0][1]*100)]
    ...
    def nFoldTest(self,fpath,nFold=5,wordFilter=None,minFreq=None,verbose=False,codec="shift-jis"):
        print "nFoldTest()"
        if not self.cateData: self.loadCategoryFile(self.cateFile_fpath,self.cateFile_index_column,self.cateFile_column_to_use)
        ...
        confMatAll = np.zeros( (len(categories), len(categories), nFold ), dtype=np.float32 )
        perm    = np.random.permutation( len(goodData) )
        for iFold in range(nFold):
            confMatOne = np.zeros( (len(categories), len(categories) ), dtype=np.float32 )
            ...
            print confMatOne
            print
            confMatAll[:,:,iFold] = confMatOne
        confMatAll = np.nanmean(confMatAll,axis=2)

        # 出力する
        if fpath:
            f = open(fpath, 'w')
            writer = csv.writer(f, lineterminator='\n')
            line = ["truth"] # vocabularies -> need to sort, thus use wordidx
            for j in range(len(categories)): line.append(categories[j])
            writer.writerow(self.encodeCodec(line,codec))
            for k in range(len(categories)):
                line = [categories[k]]
                for j in range(len(categories)): line.append(confMatAll[k,j])
                writer.writerow(self.encodeCodec(line,codec))
            f.close()
...
if __name__ == "__main__":
    ...
    wd0 = nb.wordInfo(fpath="analysis/words_all.csv")
    wd1 = nb.cleanupWords(wd0,exclude_path="data/excludeWords.csv")
    wd2 = nb.wordInfo(fpath="analysis/words_to_use.csv",wordFilter=wd1,maxEntropy=-0.45)

    #nb.TestByCompany(fpath="analysis/result.csv",topn=25,wordFilter=wd,verbose=True)
    nb.nFoldTest(fpath="analysis/5FoldTest_wd0.csv",nFold=5,wordFilter=wd0,minFreq=100)
    nb.nFoldTest(fpath="analysis/5FoldTest_wd1.csv",nFold=5,wordFilter=wd1,minFreq=100)
    nb.nFoldTest(fpath="analysis/5FoldTest_wd2.csv",nFold=5,wordFilter=wd2,minFreq=100)
